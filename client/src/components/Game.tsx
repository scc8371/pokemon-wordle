...
import { PokemonData, normalizeString, censorPortionOfString } from "../utils";
  ...
  useEffect(() => {
    ...
    switch (currHint) {
      ...
      case 1:
        ...
        hintRef.current.innerHTML += `<audio controls autoplay title='Pokemon Cry'><source src=${props.active.cry} type="audio/ogg"></audio>`;
        break;
      //displays pokemon classification
      case 2:
        const audio = hintRef.current.querySelector("audio");
        if (audio) audio.autoplay = false; // makes it so the audio doesn't autoplay constantly.

        hintRef.current.innerHTML += `<p>The ${props.active.class}</p>`;
        break;
      //shows the pokedex number of the pokemon
      case 3:
        hintRef.current.innerHTML += `<p>Pokedex Number: ${props.active.pokedexNum}</p>`;
        break;
      //shwos the pokedex entry of the pokemon.
      case 4:
        hintRef.current.innerHTML += `<p>${censorPortionOfString(props.active.pokedexEntry, props.active.name)}</p>`;
        break;

      //shows a blackened image of the pokemon
      case 5:
        hintRef.current.innerHTML += `<img src=${props.active.sprite} ref=${{ pkmnImageRef }} class='pkmn-img img-hidden'></img>`;
        break;

      //reveals the full image of the pokemon.
      case 6:
        const pkmnImg = hintRef.current.querySelector(".pkmn-img");
              ...
              onclick={() => {
                if (responseRef.current) {
                  const guess = normalizeString(
                    responseRef.current.value.trim(),
                  );

                  const pkmnName = normalizeString(props.active.name.trim());
